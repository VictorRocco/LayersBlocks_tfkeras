...
class SubPixelUpSampling2D(tf.keras.layers.Layer):
	...
	def build(self, input_shape):
		...
		if self.upsampling_factor < 1:
			raise ValueError('upsampling_factor should be integer >= 1, received: ' +str(self.upsampling_factor))

		if (self.upsampling_factor > self._subpixelupsampling_factor) and \
			(self._remainder_factor.is_integer() == False):
				raise ValueError('Number of channels ' +str(self._channels) +' not suitable for SubPixelUpSampling. ' +
								'Input shape is: ' + str(input_shape))
		...
		if self._channels < self._squared_factor:
			if self.upsampling_factor % self._subpixelupsampling_factor != 0:
				raise ValueError('Number of channels ' + str(self._channels) +
								 ...
								 str(self.upsampling_factor) + ' relation. ' +
								 'Input shape is: ' + str(input_shape))
		else:
	...
	def call(self, X):
		...
		if self.upsampling_factor == 1:
			...
			Y = UpSampling2D((self._remainder_factor, self._remainder_factor),
